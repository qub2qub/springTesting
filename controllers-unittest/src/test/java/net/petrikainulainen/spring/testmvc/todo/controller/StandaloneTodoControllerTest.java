...
//        mockMvc.perform(MockMvcRequestBuilders.get(“/Login”)).andDo(print());
    mockMvc.perform(get("/todo/add"))
        ...
        .andExpect(
            model().attribute(TodoController.MODEL_ATTRIBUTE_TODO, hasProperty("id", nullValue())))
  ...
  /**
   * Test adding EmptyTodoEntry
   */
  @Test
  public void add_EmptyTodoEntry_ShouldRenderFormViewAndReturnValidationErrorForTitle()
      throws Exception {
    ...
    mockMvc.perform(
        ...
        .andExpect(model().attributeHasFieldErrors(
            TodoController.MODEL_ATTRIBUTE_TODO,
            "title"))
  ...
  public void add_NewTodoEntry_ShouldAddTodoEntryAndRenderViewTodoEntryView() throws Exception {
    ...
    // При вызове метода сервиса на добавление из ДТО -- вернётся наш заранее подготовленный объект
    when(todoServiceMock.add(isA(TodoDTO.class))).thenReturn(added);

    String expectedRedirectViewPath =
        TestUtil.createRedirectViewPath(
            TodoController.REQUEST_MAPPING_TODO_VIEW);

    mockMvc.perform(
        post(TodoController.REQUEST_MAPPING_ADD_NEW_TODO)
            .contentType(MediaType.APPLICATION_FORM_URLENCODED)
            .param(WebTestConstants.FORM_FIELD_DESCRIPTION, "description")
            .param(WebTestConstants.FORM_FIELD_TITLE, "title")
            .sessionAttr(TodoController.MODEL_ATTRIBUTE_TODO, new TodoDTO())
    )
        .andExpect(status().isFound()) //Verify that the HTTP status code 302 is returned.
        .andExpect(view().name(expectedRedirectViewPath))
        .andExpect(redirectedUrl("/todo/1"))
        .andExpect(model().attribute(
            TodoController.PARAMETER_TODO_ID,
            is(ID.toString())))
        .andExpect(flash().attribute(
            TodoController.FLASH_MESSAGE_KEY_FEEDBACK,
            is("Todo entry: title was added.")));

    // Verify that TodoDTO object was given as a method parameter.
    // Capture the object given as a method parameter by using an ArgumentCaptor object.
  ...
  @Test
  public void deleteById_TodoEntryFound_ShouldDeleteTodoEntryAndRenderTodoListView()
      throws Exception {
    ...
    String expectedRedirectViewPath = TestUtil
        .createRedirectViewPath(TodoController.REQUEST_MAPPING_TODO_LIST);
  ...
  public void findAll_ShouldAddTodoEntriesToModelAndRenderTodoListView() throws Exception {
    ...
    Todo second = new TodoBuilder()
        .id(2L)
        .description("ToDo description v2")
        .title("Bar")
        .build();
        ...
        .andExpect(model().attribute(
            ...
            hasItems(
                ...
                allOf(
                    ...
                    hasProperty("description", is("ToDo description v2")),
   ...
   */
  @Test
  public void findById_whenTodoEntryIsFound_ShouldAddTodoEntryToModelAndRenderSingleTodoEntryView()
      throws Exception {
    ...
    mockMvc.perform(get("/todo/{id}", ID))
        ...
        .andExpect(
            model().attribute(TodoController.MODEL_ATTRIBUTE_TODO, hasProperty("id", is(1L))))
        .andExpect(model().attribute(TodoController.MODEL_ATTRIBUTE_TODO,
            hasProperty("description", is("Lorem ipsum"))))
        .andExpect(model()
            .attribute(TodoController.MODEL_ATTRIBUTE_TODO, hasProperty("title", is("Foo"))));
   ...
   */
  @Test
  public void findById_whenTodoEntryIsNotFound_ShouldRender404View() throws Exception {

    // настраиваем мок, чтобы он выбросил исключение
    when(todoServiceMock.findById(ID)).thenThrow(
        new TodoNotFoundException(
            "TestCase when Todo with id=" + ID + " is not found via service!"));

    // делаем запрос на урл по ид
    mockMvc.perform(get("/todo/{id}", ID))
        .andExpect(status().isNotFound())
        .andExpect(view().name(ErrorController.VIEW_NOT_FOUND))
        .andExpect(forwardedUrl("/WEB-INF/jsp/error/404.jsp"));
  ...
  @Test
  public void showUpdateTodoForm_TodoEntryFound_ShouldCreateFormObjectAndRenderUpdateTodoView()
      throws Exception {
  ...
  @Test
  public void update_EmptyTodoEntry_ShouldRenderFormViewAndReturnValidationErrorForTitle()
      throws Exception {
  ...
  public void update_TitleAndDescriptionAreTooLong_ShouldRenderFormViewAndReturnValidationErrorsForTitleAndDescription()
    ...
    mockMvc.perform(
        ...
        .andExpect(model().attributeHasFieldErrors(
            TodoController.MODEL_ATTRIBUTE_TODO, "title"))

        .andExpect(model().attributeHasFieldErrors(
            TodoController.MODEL_ATTRIBUTE_TODO, "description"))
  ...
  @Test
  public void update_TodoEntryFound_ShouldUpdateTodoEntryAndRenderViewTodoEntryView()
      throws Exception {
    Todo updated = new TodoBuilder()
        .id(ID)
        .description(DESCRIPTION)
    ...
    String expectedRedirectViewPath = TestUtil
        .createRedirectViewPath(TodoController.REQUEST_MAPPING_TODO_VIEW);

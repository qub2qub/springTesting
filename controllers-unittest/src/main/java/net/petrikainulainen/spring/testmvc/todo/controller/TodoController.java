...
public class TodoController {
  ...
  protected static final String REQUEST_MAPPING_ADD_NEW_TODO = "/todo/add";
  ...
  @RequestMapping(value = REQUEST_MAPPING_ADD_NEW_TODO, method = RequestMethod.GET)
  ...
  @RequestMapping(value = REQUEST_MAPPING_ADD_NEW_TODO, method = RequestMethod.POST)
  ...
  public String update(@Valid @ModelAttribute(MODEL_ATTRIBUTE_TODO) TodoDTO dto,
      BindingResult result, RedirectAttributes attributes) throws TodoNotFoundException {
    LOGGER.debug("Updating a to-do entry with information: {}", dto);

    if (result.hasErrors()) {
      LOGGER.debug(
          "Update to-do entry form was submitted with validation errors. Rendering form view.");
      return VIEW_TODO_UPDATE;
    }
  ...
  private void addFeedbackMessage(
      RedirectAttributes attributes, String messageCode, Object... messageParameters) {

    LOGGER.debug(
        "Adding feedback message with code: {} and params: {}",
        messageCode,
        messageParameters);
